...
std::string GenerateRandomString(size_t length) {
    ...
    // 使用随机数生成器
    std::mt19937 rng(static_cast<unsigned int>(std::time(nullptr))); // 初始化随机数种子
    std::uniform_int_distribution<size_t> dist(0, CHARACTERS.size() - 1); // 定义范围

    // 构建随机字符串
    std::string randomString;
    randomString.reserve(length); // 预留空间，避免多次分配
...
void NetMan::CompileWithYosys(std::string& standCellPath) {
    ...
    size_t length = 16; // 定义随机字符串长度
    std::string aig_str = "tmp/" + GenerateRandomString(length) + ".blif";
    std::string map_str = "tmp/" + GenerateRandomString(length) + ".v";
    string aigFileName(aig_str);
    string mapFileName(map_str);
